...
parser.add_argument('--start_dropout', '-drop', type=float, default=0.08, help='starting dropout rate') # 0.1 0.05 0.1
parser.add_argument('--start_weightdecay', '-weidec', type=float, default=3e-4, help='starting weightdecay rate') #0.0001, 5e-4 0.0001
...
def compute_entropy_backup(u_out, u_lab, index):
    # print("OUT", u_out)
    u_lab = labels[index]
    # print("u_lab",len(u_lab))
    u_preds = u_out.max(1)[1].type_as(u_lab)[index]
    u_prob = []
    # print("PREDS", u_preds)
    for c in range(nclass):
        correct = u_preds.eq(c).double()
        class_prob = correct.sum()/len(u_lab)
        u_prob.append(class_prob.item())
    # print("PROB", u_prob)
    entropy = 0
    for p in u_prob:
        if p != 0:
            entropy -= p * math.log(p, nclass)
    # print("E", entropy)
    # time.sleep(5)
    return entropy

def compute_entropy(u_out, u_lab, index):
    probs = u_out[index]
    # print(probs.size()) #torch.Size([500, 7])
    # print(sum(probs[0])) #tensor(1.0000)
    mean = []
    for p in probs:
        mean.append(p.detach().cpu().numpy())
    
    # mean = np.mean(mean, axis=0) #
    # print("MEAN", mean, sum(mean))
    class_num = nclass
    prob = np.array(mean)
    entropy = - prob * (np.log(prob) / np.log(class_num))
    # print("ENTRO", entropy)
    # total_un = np.sum(entropy)
...
def compute_entropy1(u_out, u_lab, index):
    probs = u_out[index]
    # print(probs.size()) #torch.Size([500, 7])
    # print(sum(probs[0])) #tensor(1.0000)
    mean = []
    ...
    # print("ENTRO", entropy)
    # total_un = np.sum(entropy)
    class_un = np.sum(entropy, axis=1) 
    total_un = np.mean(class_un)
    # class_un = entropy
    # print(total_un)
    # time.sleep(3)
    return total_un, class_un
...
def optimization_step(htensor, sampler, index, hyper=False):  
    ...
    if not hyper:
        ...
        for _ in range(avg_times):
            ...
            # print("total_h_single:",  h_single)
        #     h_all.append(h_single.detach().cpu().numpy())
        # h_avg = torch.tensor(np.mean(h_all, axis=0))
        ...
        print(t_out_avg.size())
        print(len(t_outputs), len(t_outputs[0]))
        # time.sleep(100)
        # total_uncertainty = compute_entropy(t_out_avg, labels, index)
        total_un,  total_un_class= compute_entropy(t_out_avg, labels, index)
        # print("TOTAL CLASS", total_un_class)
        ...
        for _ in range(avg_times):
            ...
            data_un,  data_un_class = compute_entropy(torch.exp(d_output), labels, index)
...
# plt.plot([i+1 for i in range(len(epsilon_list))], [max(ele) for _, ele in enumerate(epsilon_list)], "b")
# plt.plot([i+1 for i in range(len(epsilon_list))], [min(ele) for _, ele in enumerate(epsilon_list)], "r")
# plt.plot([i+1 for i in range(len(epsilon_list))], [np.mean(ele) for _, ele in enumerate(epsilon_list)], "orange")
# plt.xlabel("epoch")
# plt.ylabel("epsilon")
# plt.title("epsilon")
# plt.savefig("epsilon")
...
loss_dict = {"val_loss": val_loss_log, "val_acc": val_acc_log, "train_loss": train_loss_log, "train_acc": train_acc_log}
np.save("./save/double_"+args.dataset, loss_dict)


plt.figure(figsize=(6, 4))
plt.rcParams['axes.labelweight'] = 'bold'
plt.rcParams["font.weight"] = "bold"
plt.plot([i+1 for i in range(len(h_uncertainty))], h_uncertainty, linewidth=2, color="maroon", label="Hyper uncertainty")
plt.plot([i+1 for i in range(len(d_uncertainty))], d_uncertainty, linewidth=2, color="green", label="Data uncertatiny")
plt.plot([i+1 for i in range(len(t_uncertainty))], t_uncertainty, linewidth=2, color="royalblue", label="Total uncertainty")
...
plt.plot([i+1 for i in range(len(h_uncertainty))], h_uncertainty, "b")
plt.xlabel("epoch")
plt.ylabel("total uncertaninty")
plt.title("hyper uncertainty")
plt.savefig("Hyper U")

plt.figure(figsize=(8, 8))
plt.plot([i+1 for i in range(len(t_uncertainty))], t_uncertainty, "b")
plt.xlabel("epoch")
plt.ylabel("total uncertaninty")
...
plt.figure(figsize=(8, 8))
plt.plot([i+1 for i in range(len(d_uncertainty))], d_uncertainty, "b")
plt.xlabel("epoch")
plt.ylabel("data uncertaninty")
plt.title("data uncertainty")
plt.savefig("Hyper D")

# print(len(h_uncertainty), h_uncertainty)

# print(len(t_uncertainty), t_uncertainty)

# print(len(d_uncertainty), d_uncertainty)


double_figure(train_loss_log, train_acc_log)
