...
def create_hparams_pbt_eval(args, dropedge, dropout0, dropout1, dropout2, weightdecay, device):
    """
    Arguments:
        args: the arguments supplied by the user to the main script
        cnn_class: the convolutional net class
    ...
    htensor_list.append(s_logit(torch.tensor(dropedge), min=0.1, max=1))
    hdict['dropedge'] = HyperparameterInfo(index=len(hdict), range=(0.1, 1), hnet_fcn=lambda x: x)

    htensor_list.append(s_logit(torch.tensor(dropout0), min=0., max=0.9))
    hdict['dropout0'] = HyperparameterInfo(index=len(hdict), range=(0., 0.9), hnet_fcn=lambda x: x)

    htensor_list.append(s_logit(torch.tensor(dropout1), min=0., max=0.9))
    hdict['dropout1'] = HyperparameterInfo(index=len(hdict), range=(0., 0.9), hnet_fcn=lambda x: x)

    htensor_list.append(s_logit(torch.tensor(dropout2), min=0., max=0.9))
    hdict['dropout2'] = HyperparameterInfo(index=len(hdict), range=(0., 0.9), hnet_fcn=lambda x: x)

    htensor_list.append(s_logit(torch.tensor(weightdecay), min=1e-6, max=1e-2))
    hdict['weightdecay'] = HyperparameterInfo(index=len(hdict), range=(1e-6, 1e-2), hnet_fcn=lambda x: x)

    htensor = nn.Parameter(torch.stack(htensor_list).to(device))
    return htensor, hdict


def create_hparams_ensembale(args, dropedge, dropout0, dropout1, dropout2, weightdecay, device):
    """
    Arguments:
        args: the arguments supplied by the user to the main script
        cnn_class: the convolutional net class
