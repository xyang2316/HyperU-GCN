...
parser.add_argument('--start_dropout', '-drop', type=float, default=0.05, help='starting dropout rate') # 0.1 0.05 0.1
parser.add_argument('--start_weightdecay', '-weidec', type=float, default=5e-4, help='starting weightdecay rate') #0.0001, 5e-4 0.0001
...
def evaluate(test_adj, test_fea, index, is_warmup=True):
    ...
    hnet_tensor = hnet_transform(new_htensor.repeat(len(labels), 1), hdict)
...
if args.OOD_detection == 0:
    for i in range(10): #
        ...
        test_loss, test_acc, test_output_logit2 = evaluate(test_adj, test_fea, test_idx_filtered, True) #cora, still better
