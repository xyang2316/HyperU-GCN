...
def get_uncertainty(Baye_result):
    uncertainty = []
    uncertainty_class = []
    un_vacuity = vacuity_uncertainty(Baye_result) #model.output
...
def OOD_Detection_npz(output, dataset, model):  ## table 3
    ...
    if model == "S_GCN":
        uncertainties = get_un_sgcn(output)
    elif model == "S_BGCN_T" or model == "S_BGCN_T_K" or model == "S_BGCN" or model == "STN-GCN" or model == "MLP+STN-GCN":
        uncertainties = get_uncertainty(output)
    elif model == "DPN":
        uncertainties = get_un_dpn(output)
    elif model == "EDL":
    ...
    # print("!hereoodnpz", test_mask.sum(), len(idx_train)) #3901 80
...
def OOD_Detection(output, dataset, model):  ## table 3
    ...
    print("len(uncertainties)",len(uncertainties))
    time.sleep(5)
...
def OOD_Detection_citation(output, dataset, model):  ## table 3
    # _, _, _, _, _, _, _, test_mask, test_mask_id, test_mask_ood = load_data_ood1_acc(dataset)
    _, _, _, _, _, _, _, test_mask, test_mask_id, test_mask_ood = load_data_ood1_acc(dataset)
    ...
    print("len(uncertainties)",len(uncertainties[0]), uncertainties) # D * #un
    # time.sleep(2)

    # test_num_all = np.sum(test_mask_all)
    prediction = test_mask_ood
    # train_num = len(test_mask_all) - test_num_all
    # print("OO test_num_all", test_num_all) #2568
    print("OO test_mask", len(test_mask), test_mask.sum()) #2708 844
    # print("OO test_mask_all", test_mask_all.sum()) #2568
    # print("OO train_num", train_num) #140 (2708-2568)
    # print("prediction", prediction) #bool list
    # time.sleep(5)
    test_index = []
    auroc_s = []
    aupr_s = []
    # for i in range(10):
    #     #range(140 - 2708), random select 1000 results
    #     test_index_i = random.sample(range(int(train_num), len(test_mask)), 1000)
        # test_index.append(test_index_i)
    test_index_i = [i for i, ele in enumerate(test_mask) if ele == True]
    test_index.append(test_index_i)
    for index in test_index:
        prediction_i = prediction[index]
        # prediction_i = test_mask_ood
        # print("len(prediction_i)",len(prediction_i), prediction_i.sum(),prediction_i[:5]) #1000 316 [False False False False False]
        # print("len(index)", type(index), len(index), index[:5]) #<class 'list'> 1000 [1708, 1709, 1710, 1711, 1712]
        # time.sleep(10)
        un_roc = []
        un_pr = []
        for uncertainty in uncertainties:
            un_i = uncertainty[index]
            # print("len(un_i)", type(un_i), len(un_i), un_i[:5]) #<class 'numpy.ndarray'> 1000 [[0.99032325][0.1889076 ][0.85681725][0.09178957][0.36647138]]
            un_roc.append(roc_auc_score(prediction_i, np.array(un_i)))
            un_pr.append(average_precision_score(prediction_i, np.array(un_i)))
        auroc_s.append(un_roc)
        aupr_s.append(un_pr)
